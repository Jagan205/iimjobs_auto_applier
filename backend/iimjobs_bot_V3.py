...
def match_role(role, title, body):
    combined = f"{title} {body}"
    return match_any_phrase([role], combined, "role")


def match_jd_keywords(jd_keywords, title, body):
    if not jd_keywords:
        return False, "JD keywords list is empty"
    combined = f"{title} {body}"
    return match_any_phrase(jd_keywords, combined, "JD keyword")


def match_locations(locations, title, body):
    if not locations:
        return True, "location filter empty, allowing all locations"
    combined = f"{title} {body}"
    return match_any_phrase(locations, combined, "location")


def should_apply(role, jd_keywords, locations, title, body):
    role_ok, role_reason = match_role(role, title, body)
    keyword_ok, keyword_reason = match_jd_keywords(jd_keywords, title, body)
    location_ok, location_reason = match_locations(locations, title, body)
...
class IIMJobsBot:
    ...
    Applies only when:
      ...
      AND Location match if locations are provided
    """
    ...
    def apply_job(self, page, job_url, role):
        ...
        try:
            ...
            ok, reason = should_apply(role, self.jd_keywords, self.locations, title, body)
