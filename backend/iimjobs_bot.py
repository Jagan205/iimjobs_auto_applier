...
def build_keyword_variants(keyword: str):
    """
    Builds safe variants.

    Important:
    - No broad automatic pharmaceutical -> pharma expansion.
      If you want pharma matching, add 'pharma' explicitly in JD Keywords.
    """
...
def match_any_phrase(values, text, label, debug_title=""):
    ...
    for value in values or []:
        ...
        for v in variants:
            if phrase_match(v, text_norm):
                ...
                start = max(0, idx - 120)
                end = min(len(text_norm), idx + len(v) + 120)
                context = text_norm[start:end]

                log(f"MATCH DEBUG [{label}] title='{debug_title}' variant='{v}'")
                log(f"MATCH CONTEXT => {context}")
...
def should_apply(role, jd_keywords, locations, title, main_job_text, location_text):
    role_ok, role_reason = match_any_phrase(
        [role],
        f"{title} {main_job_text}",
        "role",
        debug_title=title,
    )

    keyword_ok, keyword_reason = match_any_phrase(
        jd_keywords,
        f"{title} {main_job_text}",
        "JD keyword",
        debug_title=title,
    ...
    if locations:
        location_ok, location_reason = match_any_phrase(
            locations,
            location_text,
            "location",
            debug_title=title,
...
class IIMJobsBot:
    ...
    Key update:
      - No broad full-page/sidebar matching.
      - No automatic pharmaceutical -> pharma expansion.
      - Adds MATCH DEBUG logs showing where a match happened.
    """
    ...
    def run(self, roles, jd_keywords=None, locations=None):
        ...
        log("Matching scope: STRICT MAIN JOB CONTENT ONLY. No full-page/sidebar matching.")
    ...
    def extract_main_job_text(self, page, title):
        """
        Strict main JD extraction.

        Avoids broad selectors like main/article/section/[class*='details'],
        because those often include recommended jobs/sidebar/filter text.
        """
        ...
        if body:
            ...
            idx = body.lower().find(title_clean.lower()) if title_clean else -1

            if idx != -1:
                window = body[idx: idx + 9000]
            else:
                window = body[:9000]
        ...
        for txt in candidates:
            ...
            if cut_positions:
                txt = txt[:min(cut_positions)]

            txt = clean_text(txt)

            if len(txt) >= 250:
                cleaned_candidates.append(txt)
        ...
        return clean_text(best[:7000])

    def extract_location_text(self, page, title, main_job_text):
        snippets = [title, main_job_text[:1200]]
    ...
    def apply_job(self, page, job_url, role):
        ...
        try:
            ...
            log(f"Main job text preview: {main_job_text[:500]}")
            log(f"Location text preview: {location_text[:250]}")

            if not main_job_text or len(main_job_text) < 250:
                reason = "main job text extraction failed or too short"
                log(f"SKIPPED - {reason}")
                self.record_job(job_url, title, role, status="skipped_extraction_failed", reason=reason)
                runtime_state["skipped"] += 1
                return
